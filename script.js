...
function inserirAluno(total_aulas){
    ...
    //JSON = JOAQUIM SOUZA OLIVEIRA NASCIMENTO

    var aluno = {}

    aluno.matricula = prompt('Digite a matrícula do aluno');
    aluno.nome = prompt('Digite o nome do aluno');
    aluno.faltas = parseInt(prompt('Digite o numero de faltas do aluno'));
    aluno.notab1 = parseFloat(prompt('Digite a nota do primeiro bimestre do aluno'));
    aluno.notab2 = parseFloat(prompt('Digite a nota do segundo bimestre do aluno'));
    aluno.mediaparcial = (aluno.notab1 + aluno.notab2) / 2;

    var percentual_faltas = aluno.faltas / total_aulas;

    if (percentual_faltas > 0.25) {
        aluno.situacao = 'REPROVADO - FALTA';
        aluno.notapr = '-'
        ...
        if(aluno.mediaparcial >= 7) {
            ...
            aluno.notapr = parseFloat(prompt('Digite a nota da prova de recuperação do aluno'));
            aluno.mediafinal = (aluno.mediaparcial + aluno.notapr) / 2;
            if (aluno.mediafinal >= 5) {
                aluno.situacao = 'APROVADO';
            } else {
                aluno.situacao = 'REPROVADO - NOTA'
            }
        }
    }
